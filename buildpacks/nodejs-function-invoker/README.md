# Node.js Function Invoker Cloud Native Buildpack

Heroku's Cloud Native Buildpack for [@heroku/sf-fx-runtime-nodejs](https://github.com/forcedotcom/sf-fx-runtime-nodejs) and [Salesforce Functions](https://developer.salesforce.com/docs/platform/functions/overview).

[![CI](https://github.com/heroku/buildpacks-nodejs/actions/workflows/ci.yml/badge.svg)](https://github.com/heroku/buildpacks-nodejs/actions/workflows/ci.yml)

[![Registry](https://img.shields.io/badge/dynamic/json?url=https://registry.buildpacks.io/api/v1/buildpacks/neetodeploy/nodejs-function-invoker&label=version&query=$.latest.version&color=DF0A6B&logo=data:image/png;base64,&labelColor=white)](https://registry.buildpacks.io/buildpacks/neetodeploy/nodejs-function-invoker)

This buildpack is designed to work in conjunction with [`neetodeploy/nodejs-engine`](../nodejs-engine/README.md) (to install `node`) and [`neetodeploy/nodejs-npm`](../npm/README.md) (to install any dependencies). This buildpack is part of the [`neetodeploy/nodejs-function`](../../meta-buildpacks/nodejs-function/README.md), which is the default and recommended Clound Native Buildpack for Node.js Salesforce Functions.

## What it does

This buildpack installs `@heroku/sf-fx-runtime-nodejs` from npm, makes `sf-fx-runtime-nodejs` available on `$PATH`, and sets `sf-fx-runtime-nodejs serve` as the default process type.

## Reference

### Detect

This buildpack expects a `function.toml` to exist in the project root or a `project.toml` with a `com.salesforce.type = "function` field to pass detection.

### Build Plan

This buildpack `requires` `node` to be provided by another buildpack. This builpack `provides` and `requires` `nodejs-function-invoker`.

### Environment Variables

#### PATH

`$PATH` will be modified such that `sf-fx-runtime-nodejs` is available.

## Usage

To build an app locally into an OCI Image with this buildpack, use the `pack` command from Cloud Native Buildpacks:

```
pack build example-fn-image --buildpack neetodeploy/nodejs-engine --buildpack neetodeploy/nodejs-npm --buildpack neetodeploy/nodejs-function-invoker --path /some/example-fn
```

## Additional Info

For development, dependencies, contribution, license and other info, please refer to the [root README.md](../../README.md).
