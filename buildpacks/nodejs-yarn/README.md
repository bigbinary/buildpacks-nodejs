...
Use the `neetodeploy/nodejs-corepack` buildpack to install yarn. It will install
...
[neetodeploy/nodejs-engine](../nodejs-engine) buildpack and this one:

```
pack build example-app-image --buildpack neetodeploy/nodejs-engine --buildpack neetodeploy/nodejs-yarn --path /some/example-app
```
